...
const tables = [
  {
    ...
    ids: [142, 26, 28, 29, 34, 38, 110, 196],
    icon: Car,
  },
  {
    name: "Manufacturer Details",
    ids: [27, 31, 75, 76, 77, 10],
    icon: Building2,
  },
  {
...
const VinLookupTable = ({ vinData }: { vinData: VinLookupResponse }) => {
  ...
  const renderTable = (
    results: VinLookupResponse["data"],
    title: string,
    Icon: any = MoreHorizontal
  ) =>
    results &&
    results.length > 0 && (
      <div key={title} className="mb-6">
        <button
          ...
          className={cn(
            ...
            expandedTables.includes(title) && "rounded-b-none"
          )}
        >
